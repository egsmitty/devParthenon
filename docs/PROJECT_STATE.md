...
**Features**
- Settings panel: theme (light parchment mode), motion toggle, font scale,
  sound on/off.
- Progress export/import (JSON) and a "share my temple" snapshot image.
- Streaks / daily goal / XP; a stats dashboard (per-concept mastery heatmap).
- Real audio (bundled, CSP-safe): page turn, stone-set, torch ambience — with
  a mute toggle and reduced-motion-style "reduced sound" respect.
